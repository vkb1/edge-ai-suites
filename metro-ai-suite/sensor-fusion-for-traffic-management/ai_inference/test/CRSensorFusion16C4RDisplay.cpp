    ...
    if (jsonTree.count("latency") > 0) {
        ...
        for (auto roi_i = roi_info.begin(); roi_i != roi_info.end(); ++roi_i) {
            ...
            if ((x == 0 && y == 0 && w == 0 && h == 0) || (-1 == sensor_source)) {
                ...
                cv::Point start_point(x_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4), y_resize + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
                cv::Point end_point(x_resize + w_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4), y_resize + h_resize + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
                cv::Point start_point2(x_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4), max(10, y_resize - 6) + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
