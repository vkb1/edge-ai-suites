...
int main(int argc, char **argv)
{
    try {
        ...
        for (unsigned j = 0; j < gpu_streams; ++j) {
            ...
            std::thread d(OSDload, std::ref(pipeline_repeatinputs), totalStreamNum, display_idx, windowImage, params, msg_buffer_gpu[j], std::ref(pl), mediaType, displayType,
        ...
        for (unsigned j = 0; j < npu_streams; ++j) {
            ...
            std::thread d(OSDload, std::ref(pipeline_repeatinputs), totalStreamNum, display_idx, windowImage, params, msg_buffer_npu[j], std::ref(pl), mediaType, displayType,
                          showMetrics, saveFlag, logoFlag);
            dThs.push_back(std::move(d));
            display_idx++;
        }
