...
def test_gen_chart():
    ...
    assert helm_utils.generate_helm_chart(chart_path, constants.WELD_SAMPLE_APP) == True, "Failed to generate helm chart."  # nosec B101
...
def test_blank_values():
    ...
    assert helm_utils.uninstall_helm_charts(release_name_weld, namespace) == True, "Failed to uninstall Helm release if exists."  # nosec B101
    ...
    assert helm_utils.helm_install(release_name_weld, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN) == False  # nosec B101
...
def test_invalid_values():
    ...
    assert helm_utils.helm_install(release_name_weld, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN, sample_app=constants.WELD_SAMPLE_APP) == False  # nosec B101
...
def test_verify_pods_stability_after_udf_activation(setup_helm_weld_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_ts_logs(namespace, "INFO") == True, "Failed to verify INFO logs in pod logs"  # nosec B101
    logger.info("Pod logs show INFO messages as expected")

    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, constants.WELD_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
...
def test_verify_pods_stability_after_influxdb_restart(setup_helm_weld_environment, telegraf_input_plugin):
    ...
    assert helm_utils.pod_restart(namespace) == True, "Failed to restart pod for mqtt input plugin."  # nosec B101
...
def test_mqtt_alerts(setup_helm_weld_environment, telegraf_input_plugin):
    ...
    # Set up MQTT alerts
    assert helm_utils.setup_mqtt_alerts(chart_path, constants.WELD_SAMPLE_APP) == True, "Failed to set up MQTT alerts."  # nosec B101
    logger.info("MQTT alerts are set up successfully")
    time.sleep(3)
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for MQTT input plugin."  # nosec B101
    logger.info("All pods are running for mqtt input plugin")
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, sample_app=constants.WELD_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
    # Get the current system time
    logger.info(f"Wait for the application to run for {wait_time} seconds...")
    time.sleep(wait_time)  # Wait for the pods to stabilize
    logger.info("Verifying pod logs for alerts...")
    assert helm_utils.verify_ts_logs_alerts(namespace, "mqtt_weld") == True, "Failed to verify pod logs for mqtt input plugin."  # nosec B101
...
def test_verify_pods_logs_with_respect_to_log_level():
    ...
    assert helm_utils.uninstall_helm_charts(release_name_weld, namespace) == True, "Failed to uninstall Helm release."  # nosec B101
    logger.info("Helm release is uninstalled if it exists")
    ...
    assert helm_utils.helm_install(release_name_weld, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN, sample_app=constants.WELD_SAMPLE_APP) == True, "Failed to install Helm release."  # nosec B101
    ...
    assert helm_utils.helm_upgrade(release_name_weld, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN) == True, "Failed to upgrade Helm release."  # nosec B101
...
def test_helm_upgrade(setup_helm_weld_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_pods(namespace) is True, "Initial deployment failed"  # nosec B101
    ...
    assert helm_utils.helm_upgrade(release_name_weld, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN) is True, \
        "Helm upgrade failed"  # nosec B101
    ...
    assert helm_utils.verify_pods(namespace) is True, "Pods not healthy after upgrade"  # nosec B101
    ...
    assert upgraded_pod_count == initial_pod_count, \
        f"Pod count mismatch: expected {initial_pod_count}, got {upgraded_pod_count}"  # nosec B101
    ...
    for service in expected_services:
        ...
        assert found, f"Service '{service}' missing after upgrade"  # nosec B101
