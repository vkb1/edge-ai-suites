...
def test_authentication_influx_grafana(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_pods(namespace) == True, "Failed to verify pods in the namespace."  # nosec B101
    # Access the test cases dictionary
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path) == True, "Failed to activate UDF deployment package."  # nosec B101
    ...
    assert security_utils.influxdb_login(namespace, chart_path) == True, "Failed to login to InfluxDB with provided credentials."  # nosec B101
    ...
    assert asyncio.run(security_utils.login_to_grafana(grafana_url, grafana_username, grafana_password)) == True, "Failed to login to Grafana with provided credentials."  # nosec B101
...
def test_nmap_open_ports_opcua(setup_helm_environment, telegraf_input_plugin):
    ...
    assert security_utils.check_nmap(target, exposed_ports) == True, "Failed to find open ports on the target."  # nosec B101
...
def test_creds_in_pod_logs(setup_helm_environment, telegraf_input_plugin):
    ...
    assert security_utils.verify_pods_creds(namespace, influxdb_creds, grafana_creds) == True, "Credentials found in pod logs."  # nosec B101

def test_data_integrity():
    logger.info("TC_005: Verify data integrity in the system.")
    assert security_utils.verify_docker_file_integrity() == True, "Docker-compose file integrity verification failed."  # nosec B101
    assert security_utils.verify_helm_file_integrity() == True, "Helm files integrity verification failed."  # nosec B101
    ...
    assert helm_utils.helm_install(release_name, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN, continuous_simulator_ingestion="false") == True, "Failed to install Helm release."  # nosec B101
    time.sleep(5)  # Wait for the pods to stabilize
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods."  # nosec B101
    ...
    assert first_wind_speed is not None or last_wind_speed is not None or total_records is not None, "Failed to fetch wind turbine data."  # nosec B101
    ...
    assert security_utils.verify_data_integrity_influxdb(chart_path, namespace, first_wind_speed, last_wind_speed, total_records) == True, "Data integrity verification failed in InfluxDB."  # nosec B101
    time.sleep(5)
    assert helm_utils.uninstall_helm_charts(release_name, namespace) == True, "Failed to uninstall Helm release after data integrity test."  # nosec B101
    assert helm_utils.check_pods(namespace) == True, "Pods are still running after cleanup."  # nosec B101
...
def test_authentication_influx_grafana_docker(setup_docker_environment):
    ...
    assert security_utils.influxdb_login_docker() == True, "Failed to login to InfluxDB with provided credentials."  # nosec B101
    ...
    logger.info("Successfully retrieved Grafana credentials.")
    assert asyncio.run(security_utils.login_to_grafana_docker(port=context["docker_grafana_port"])) == True, "Failed to login to Grafana with provided credentials."  # nosec B101
...
def test_verify_sensitive_data_in_mqtt_logs(setup_docker_environment):
    ...
    assert results["success"], f"Credential verification failed at step: {results.get('failed_step', 'unknown')}"  # nosec B101
    ...
    assert results["env_variables_read"], "Failed to read environment variables"  # nosec B101
    assert results["deployment_success"], "Failed to deploy containers"  # nosec B101
    assert results["containers_up"], "Containers are not running"  # nosec B101
    assert results["credentials_in_logs"], "Credentials not found in container logs"  # nosec B101
