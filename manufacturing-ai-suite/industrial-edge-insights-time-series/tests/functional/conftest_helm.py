...
def setup_helm_environment(request):
    ...
    assert helm_utils.helm_install(release_name, chart_path, namespace, telegraf_input_plugin, sample_app=sample_app) == True, "Failed to install Helm release."  # nosec B101
    
    # Wait for pods to be ready before yielding to tests
    logger.info(f"Waiting for pods to be ready in namespace '{namespace}'...")
    assert helm_utils.verify_pods(namespace, timeout=300) == True, "Failed to verify pods are running after installation."  # nosec B101
...
def setup_helm_weld_environment(request):
    ...
    assert helm_utils.helm_install(release_name_weld, chart_path, namespace, telegraf_input_plugin, sample_app=sample_app) == True, "Failed to install Helm release."  # nosec B101
...
def setup_multimodal_helm_environment():
    ...
    assert helm_utils.uninstall_helm_charts(release_name_multi, namespace_multi) == True, "Failed to uninstall multimodal Helm release if exists."  # nosec B101

    case = helm_utils.password_test_cases["test_case_3"]
    values_yaml_path = os.path.expandvars(chart_path_multi + '/values.yaml')
    assert helm_utils.update_values_yaml(values_yaml_path, case) == True, "Failed to update multimodal values.yaml."  # nosec B101
    ...
    assert helm_utils.helm_install(release_name_multi, chart_path_multi, namespace_multi, constants.TELEGRAF_MQTT_PLUGIN) == True, "Failed to install multimodal Helm release."  # nosec B101
    time.sleep(3)
    yield
    assert helm_utils.uninstall_helm_charts(release_name_multi, namespace_multi) == True, "Failed to uninstall multimodal Helm release if exists."  # nosec B101
    # Use shorter timeout for cleanup and make it non-blocking for CI/CD
