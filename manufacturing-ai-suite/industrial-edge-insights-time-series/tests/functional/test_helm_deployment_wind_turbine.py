...
def test_gen_chart():
    ...
    assert helm_utils.generate_helm_chart(chart_path) == True, "Failed to generate helm chart."  # nosec B101
...
def test_blank_values():
    ...
    assert helm_utils.uninstall_helm_charts(release_name, namespace) == True, "Failed to uninstall Helm release if exists."  # nosec B101
    ...
    assert helm_utils.helm_install(release_name, chart_path, namespace, constants.TELEGRAF_OPCUA_PLUGIN) == False  # nosec B101
...
def test_verify_pods_all_running_opcua_switch_to_mqtt(setup_helm_environment):
    ...
    assert helm_utils.uninstall_helm_charts(release_name, namespace) == True, "Failed to uninstall Helm release."  # nosec B101
    logger.info("Helm release is uninstalled if it exists")
    assert helm_utils.check_pods(namespace) == True, "Pods are still running after cleanup."  # nosec B101
    ...
    assert helm_utils.helm_install(release_name, chart_path, namespace, constants.TELEGRAF_OPCUA_PLUGIN, sample_app=sample_app) == True, "Failed to install Helm release."  # nosec B101
    logger.info("Helm is installed for opcua input plugin")
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for OPC UA input plugin."  # nosec B101
    logger.info("All pods are running for opcua input plugin")
    assert helm_utils.helm_uninstall(release_name, namespace) == True, "Failed to uninstall Helm release."  # nosec B101
    logger.info("Helm is uninstalled for opcua input plugin")
    assert helm_utils.helm_install(release_name, chart_path, namespace, constants.TELEGRAF_MQTT_PLUGIN) == True, "Failed to install Helm release."  # nosec B101
    logger.info("Helm is installed for mqtt input plugin")
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for MQTT input plugin."  # nosec B101
...
def test_verify_pods_opcua_for_5mins(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for OPC UA input plugin."  # nosec B101
    logger.info("All pods are running for opcua input plugin")
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, sample_app=constants.WIND_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
    ...
    assert helm_utils.verify_pods_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for OPC UA input plugin."  # nosec B101
...
def test_verify_pods_stability_after_udf_activation(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_pods_logs(namespace, "error") == False, "Pod logs didn't show error type logs when udf package is not activated"  # nosec B101
    ...
    time.sleep(wait_time)  # Wait for the pods to stabilize
    assert helm_utils.verify_pods_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for opcua input plugin."  # nosec B101
...
def test_verify_pods_stability_after_influxdb_restart(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.pod_restart(namespace) == True, "Failed to restart pod for opcua input plugin."  # nosec B101
    logger.info("Pod is restarted for opcua input plugin")
    time.sleep(1)
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for opcua input plugin."  # nosec B101
    logger.info("All pods are running for opcua input plugin")
    assert helm_utils.verify_pods_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for opcua input plugin."  # nosec B101
    logger.info("Pod logs are verified for opcua input plugin")
...
def test_mqtt_alerts(setup_helm_environment, telegraf_input_plugin):
    ...
    # Set up MQTT alerts
    assert helm_utils.setup_mqtt_alerts(chart_path) == True, "Failed to set up MQTT alerts."  # nosec B101
    ...
    assert helm_utils.verify_ts_logs_alerts(namespace, "mqtt") == True, "Failed to verify pod logs for mqtt input plugin."  # nosec B101
...
def test_verify_pods_mqtt_for_5mins(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_pods_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for MQTT input plugin."  # nosec B101
    logger.info("Pods logs are working for mqtt input plugin")
...
def test_opcua_alerts(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.setup_opcua_alerts(chart_path) == True, "Failed to set up OPC UA alerts."  # nosec B101
    logger.info("OPC UA alerts are set up successfully")
    
    # Copy UDF package and activate
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, alert_mode="opcua") == True, "Failed to activate UDF deployment package."  # nosec B101
    logger.info("UDF package copied and activated successfully")
    ...
    assert helm_utils.restart_deployment(namespace, "opcua-server") is True, "Failed to restart OPC UA server deployment."  # nosec B101
    ...
    assert helm_utils.verify_ts_logs_alerts(namespace, "opcua") is True, "Failed to verify pod logs for opcua input plugin."  # nosec B101
    logger.info("Pods logs are working for opcua input plugin")
...
def test_verify_pods_logs_with_respect_to_log_level(setup_helm_environment, telegraf_input_plugin):
    ...
    time.sleep(3)  # Wait for the pods to stabilize
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for Case 4."  # nosec B101
    logger.info("All pods are running")
    time.sleep(wait_time)  # Wait for the pods to stabilize
    assert helm_utils.verify_ts_logs(namespace, "ERROR") == False, "Failed to verify pod logs for ERROR log level."  # nosec B101
    ...
    assert helm_utils.helm_upgrade(release_name, chart_path, namespace, constants.TELEGRAF_OPCUA_PLUGIN) == True, "Failed to upgrade Helm release."  # nosec B101
    logger.info("Helm is updated for Case 3: DEBUG log level")

    time.sleep(3)  # Wait for the pods to stabilize
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for Case 3."  # nosec B101
    logger.info("All pods are running")

    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, sample_app=constants.WIND_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
    logger.info(f"UDF deployment package is activated and Wait for {wait_time} seconds for pods to stabilize")
    time.sleep(wait_time)  # Wait for the pods to stabilize
    assert helm_utils.verify_ts_logs(namespace, "DEBUG") == True, "Failed to verify pod logs for DEBUG log level."  # nosec B101
    ...
    time.sleep(3)  # Wait for the pods to stabilize
    assert helm_utils.verify_pods(namespace) is True, "Failed to verify pods for Case 5."  # nosec B101
    logger.info("All pods are running")
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, sample_app=constants.WIND_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
    logger.info(f"UDF deployment package is activated and waiting for {wait_time} seconds for pods to stabilize")
    time.sleep(wait_time)  # Wait for the pods to stabilize
    assert helm_utils.verify_ts_logs(namespace, "INFO") == True, "Failed to verify pod logs for INFO log level."  # nosec B101
...
def test_influxdb_data_with_opcua(setup_helm_environment, telegraf_input_plugin):
    ...
    assert helm_utils.verify_ts_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for OPC UA input plugin."  # nosec B101
    ...
    assert helm_utils.verify_influxdb_connectivity(namespace, chart_path) is True, "Failed to verify InfluxDB connectivity for OPC UA input plugin."  # nosec B101
...
def test_influxdb_data_with_mqtt(setup_helm_environment, telegraf_input_plugin):
    ...
    # Copy UDF package and activate - ConfigMap was already patched during helm_install
    # The API restart endpoint will stop Kapacitor, install UDF pip packages, then start Kapacitor
    assert helm_utils.setup_sample_app_udf_deployment_package(chart_path, alert_mode="mqtt") == True, "Failed to activate UDF deployment package."  # nosec B101
    logger.info("UDF package copied and activated successfully")
    ...
    assert helm_utils.wait_for_mqtt_sample(namespace, constants.WIND_TURBINE_INGESTED_TOPIC, timeout=120) == True, \
        "Failed to observe MQTT data before InfluxDB verification."  # nosec B101
    logger.info(f"MQTT data confirmed. Waiting additional {wait_time}s for data to flow to InfluxDB...")
    time.sleep(wait_time)
    
    assert helm_utils.verify_ts_logs(namespace, "DEBUG") is True, "Failed to verify pod logs for MQTT input plugin."  # nosec B101
    # InfluxDB connectivity test (simplified approach aligned with 08Weekly)
    logger.info("Verifying basic InfluxDB connectivity for mqtt input plugin")
    assert helm_utils.verify_influxdb_connectivity(namespace, chart_path) is True, "Failed to verify InfluxDB connectivity for MQTT input plugin."  # nosec B101
...
def test_gpu_opcua_helm(setup_helm_environment, request, telegraf_input_plugin):
    ...
    assert helm_utils.setup_sample_app_udf_deployment_package(actual_chart_path, sample_app=constants.WIND_SAMPLE_APP) == True, "Failed to activate UDF deployment package."  # nosec B101
    logger.info("UDF deployment package is activated")
    ...
    assert curl_result, "GPU configuration test via REST API failed"  # nosec B101
